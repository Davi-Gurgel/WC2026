          ...
          <>
              ...
              {columns.map(([title, matches]) => (
                <div className="flex flex-col min-w-60 snap-center shrink-0 min-h-[600px] justify-between py-10 relative group" key={title}>
                  ...
                  {matches.length ? (
                    ...
                    <div className="flex h-20 items-center justify-center rounded-sm border border-dashed border-white/10 bg-white/1 text-xs font-mono text-white/20">
                      AWAITING DATA
                    </div>
                  )}
                </div>
            ...
            {!state.champion && (
              <div className="fixed bottom-10 left-1/2 z-40 -translate-x-1/2">
                <button 
                  className="rounded-none bg-linear-to-r from-wc-blue via-wc-red to-wc-green px-8 py-4 font-outfit text-sm font-black uppercase tracking-[0.2em] text-white shadow-2xl transition-transform hover:-translate-y-1 active:scale-95 disabled:opacity-50 disabled:pointer-events-none" 
...
const BracketMatch = memo(function BracketMatch({ match, final = false }: { match: Match; final?: boolean }) {
